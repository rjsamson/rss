...
func TestParse(t *testing.T) {
	...
	assert.Nil(err)
	assert.Equal("FeedForAll Sample Feed", feed.Title)
	assert.Equal("http://www.feedforall.com/industry-solutions.htm", feed.Link)

	image := feed.Image
	assert.Equal("http://www.feedforall.com/ffalogo48x48.gif", image.Url)
	assert.Equal(48, image.Height)
	assert.Equal(48, image.Width)

	for index, item := range feed.Items {
		assert.Equal(titles[index], item.Title)
		assert.Equal(links[index], item.Link)
	}
}

var titles = []string{"RSS Solutions for Restaurants",
	"RSS Solutions for Schools and Colleges",
	"RSS Solutions for Computer Service Companies",
	"RSS Solutions for Governments", "RSS Solutions for Politicians",
...
var links = []string{"http://www.feedforall.com/restaurant.htm",
